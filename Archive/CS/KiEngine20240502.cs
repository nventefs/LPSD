...
namespace nVentErico.LPSD.Controllers.Cvm
{
    ...
    public class KiEngine
    {
        ...
        private void Formula_B()
        {
            ...
            ki = (0.55 * Math.Pow(H, 0.544) * Math.Pow(W, -0.14) * Math.Pow(R_c, -0.367) + 1);
        ...
        private void Formula_D()
        {
            ...
            ki = 0.56 * (Math.Pow(H, 0.82) * Math.Pow(W, -0.31) * Math.Pow(R_c, -0.45)) + 1;
        ...
        private void Formula_K()
        {
            ...
            ki = 0.84 * Math.Pow(H, 0.474) * Math.Pow(W, -0.109) * Math.Pow(H_f, -0.326) + 0.27;
        ...
        private void Formula_L()
        {
            ...
            if (Terminal != null)
            {
                H_f = Terminal.Top.Z - BaseLevelOfBuilding.ForgeLevelElevation;
            }
            else
            {
                H_f = Point.Position.Z - BaseLevelOfBuilding.ForgeLevelElevation;
        ...
        private void Formula_M()
        {
            ...
            {
                H_f = Terminal.Top.Z - Terminal.Level.ForgeLevelElevation;
                H = Terminal.Level.ForgeLevelElevation;
                W = Terminal.Level.BoundingBox.MinimumWidth;
            }

            // K_i at Structure = 0.68 𝐻^0.78 𝑊^(−0.28) 𝐻_𝑓^(−0.43) + 0.45
            double ki = 0.68 * Math.Pow(H, 0.78) * Math.Pow(W, -0.28) * Math.Pow(H_f, -0.43) + 0.45;
        ...
        private void Formula_O()
        {
            ...
            double W;
            double H;
            if ((Terminal.Level.LevelNumber == BaseLevelOfBuilding.LevelNumber) | Terminal.PoiPoint.IsExtendedCornerOrEdge)
            {
                H_f = Terminal.Top.Z - Terminal.Level.ForgeLevelElevation;
                W = Terminal.Level.BoundingBox.MinimumWidth;
                H = Terminal.Level.ForgeLevelElevation;
            ...
            double P = Terminal.PoiPoint.Slope;
        ...
        private void Formula_U()
        {
            ...
            FIR = 0.9 * Math.Pow(H, 0.25) * Math.Pow(W, -0.13) + 0.2;
        ...
        private void Formula_Y()
        {
            ...
            double Fir_Max = 1.8 * Math.Pow(H + W, -0.12) + 1;

            //〖FIR〗_min = 0.9H ^ 0.25 W ^ (-0.13) + 0.2
            double Fir_Min = 0.9 * Math.Pow(H, 0.25) * Math.Pow(W, -0.13) + 0.2;

            // FIR = ((〖FIR〗_max -〖FIR〗_min)/ 2)P +〖FIR〗_min
            double FIR = ((Fir_Max - Fir_Min) / 2) * P + Fir_Min;
